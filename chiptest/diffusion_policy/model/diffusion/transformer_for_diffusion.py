...
class TransformerForDiffusion(ModuleAttrMixin):
    def __init__(
        ...
        if T_cond > 0:
            ...
            if n_cond_layers > 0:
                encoder_layer = nn.TransformerEncoderLayer(
                    ...
                    dropout=p_drop_attn,
                    activation="relu",  # 10/21 将这里改成了relu（原来使用gelu激活函数）
                    batch_first=True,
                    norm_first=True,
                )
            ...
            decoder_layer = nn.TransformerDecoderLayer(
                ...
                init=False,  # 10/21 根据库的修改，增加了init参数
            )
            ...
            # 10/19 由于之前修改了transformer库里的东西，所以这里增加一个first_layer
        else:
            # encoder only BERT
            encoder_only = True
        ...
        self.ln_f = nn.LayerNorm(
            n_emb, elementwise_affine=False
        )  # 这里elementwise_affine不能是True，Layernorm层不能有weight和bias
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Embedding)):
            ...
            pass  # 需要的模型Layernorm层不支持bias和weight
            # torch.nn.init.zeros_(module.bias)
